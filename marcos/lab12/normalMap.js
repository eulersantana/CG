...
// ********************************************************
// ********************************************************
function drawAxis(o, shaderProgram, primitive) {
...
function draw(o, shaderProgram, primitive) {
	...
	if (texture[texNormMap*2] != null) {   	
		gl.activeTexture(gl.TEXTURE0);
		gl.bindTexture(gl.TEXTURE_2D, texture[texNormMap*2]);
	}

	if (texture[texNormMap*2+1] != null) {   	
		gl.activeTexture(gl.TEXTURE1);
		gl.bindTexture(gl.TEXTURE_2D, texture[texNormMap*2+1]);
	}
...
function drawScene() {
	...
	drawAxis(axis, simpleShader, gl.LINES);	
	...
	gl.uniformMatrix4fv(textShader.uMVPMat, false, mvpMat.elements);
	...
	for(var o = 0; o < model.length; o++) 
		draw(model[o], textShader, gl.TRIANGLES);		
}
...
function webGLStart() {
	...
	canvas					= document.getElementById("normalMapping");
	...
	textShader.vNormalAttr 		= 0;//gl.getAttribLocation(textShader, "aVNormal");
	...
	textShader.uMVPMat 			= gl.getUniformLocation(textShader, "uMVPMat");
	textShader.uNormalMat 		= gl.getUniformLocation(textShader, "uNormalMat");
	...
	readOBJFile("../../modelos/cubeText_NM.obj", 1, true);
	...
	var tick = function() {   // Start drawing
		...
		if ( (model.length > 0) && (textureOK >= g_drawingInfo.mtl.length*2) )
			drawScene();
		else 
