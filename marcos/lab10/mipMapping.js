...
function initTexture(textureName) {
	...
	image.onload = function() {
		...
		console.log("Texture = " + image.width + " x " + image.height);
		textureOK = true;
		}
	image.src = textureName;
	console.log("Texture filename = " + image.src);
}
...
var groupModel = null;
	...
	if (groupModel.Material.mapKd != "") {
		console.log("mapKD = " + groupModel.Material.mapKd);
		initTexture(groupModel.Material.mapKd);
		}
	else {
		console.log("No mapKD = ." + groupModel.Material.mapKd + ".");
		textureOK = true;
		}
}
...
function handleKeyDown(event) {
	...
	switch (keyunicode) {
		...
		case 38	:	// Up cursor key
					cameraPos.elements[0] += 0.05; 
					cameraPos.elements[1] += 0.05;
					cameraPos.elements[2] += 0.05;
					break;
...
function webGLStart() {
	...
	canvas					= document.getElementById("mipMapping");
	...
	textShader = initShaders("mipMapping", gl);	
	if (!textShader) {
	...
	readOBJFile("../../modelos/cubeTexture.obj", gl, 1, true);
