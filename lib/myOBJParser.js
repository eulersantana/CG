...
// DrawInfo Object
//------------------------------------------------------------------------------
var DrawingInfo = function(vertices, colors, normals, textCoord, indices, BBox) {
...
var BB = new BBox(	this.vertices[0].x, this.vertices[0].y, this.vertices[0].z,
	...
	if (this.normals.length == 0) {
		...
		var normalPerVertex	= new Float32Array(this.vertices.length * 3);
		var aTangent		= new Float32Array(this.vertices.length * 3);
		var aBiTangent		= new Float32Array(this.vertices.length * 3);
		var vValency		= new Array(this.vertices.length);
	...
	for (o=0 ; o < this.objects.length ; o++) {
		for(var g = 0; g < this.objects[o].groups.length; g++) {
			...
			var matGrp = this.findMaterial(grp.materialName);
			
			if (matGrp == null) 			
				console.log("Group material not found => ." + grp.materialName + ".");
			...
			for(var f = 0; f < grp.faces.length; f++) {			
				...
				for(var i = 0; i < face.vIndices.length; i++) {				
					...
					if (matGrp != null) {			
						colors[newIndex * 4 + 0] = matGrp.Kd.r;
						colors[newIndex * 4 + 1] = matGrp.Kd.g;
						colors[newIndex * 4 + 2] = matGrp.Kd.b;
						colors[newIndex * 4 + 3] = matGrp.Kd.a;
						}
					else {
						colors[newIndex * 4 + 0] = 
						colors[newIndex * 4 + 1] = 
	...
	return new DrawingInfo(aVertices, aColor, aNormal, aTextures, aIndices, BB);
}
