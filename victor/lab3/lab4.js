...
function drawScene(gl, shader) {
	...
	gl.viewport(0, 0, gl.viewportWidth * 1/3, gl.viewportWidth * 1/3);
	gl.uniform1i(shader.SamplerCor, 1);
	gl.drawArrays(gl.TRIANGLES, 0, triangleVertexPositionBuffer.numItems);

	gl.viewport(gl.viewportWidth * 1/3, 0, gl.viewportWidth * 1/3, gl.viewportWidth * 1/3);
	gl.uniform1i(shader.SamplerCor, 2);
	gl.drawArrays(gl.TRIANGLES, 0, triangleVertexPositionBuffer.numItems);

	gl.viewport(gl.viewportWidth * 2/3, 0, gl.viewportWidth * 1/3, gl.viewportWidth * 1/3);
...
function initTexture(gl, shader) {
	...
	image.onload = function(){
		...
		text.innerHTML 		= 	"Imagem :" + image.src + 
								...
								" <i>x</i> " + image.width +
								"<br/> Dimensao Canvas = " + canvas.height +
								" <i>x</i> " + canvas.width;		
