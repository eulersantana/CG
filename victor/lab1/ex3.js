...
// ********************************************************
// ********************************************************
function changeRSize(r) {
	var text = document.getElementById("outputR");
	text.innerHTML = "Radius Size = " + r;
	vRSize = r
	vPos = new Array
	vColor = new Array
	build2DGrid(30,30)
...
var dy = 2.0/ny;
	...
	for(i=0 ; i <= nx ; i++) {
		for (j=0 ; j <= ny ; j++) {
			...
			if (Math.sqrt(x + y) < vRSize)
			// Talvez o raio (0.2) seja 0.5, mas na especificação esta 0.2
			{
				x = 0
				y = 0
			}
...
function webGLStart() {
	...
	slider = document.getElementById("rSize");
	vRSize = slider.value
