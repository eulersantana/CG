...
function initGL(canvas) {
	...
	gl.viewportWidth 	= 700;
	gl.viewportHeight 	= 700;
...
var vTex = new Array;
	...
	/*Vetor de nromalização da imagem, em um vetor de textura. Sendo assim a imagem sera desenhada no canvas*/
...
function drawScene(gl, shader) {
	gl.viewport(0,  gl.viewportWidth/3, gl.viewportWidth, gl.viewportHeight);
	...
	gl.viewport(0,  0, gl.viewportWidth/3, gl.viewportHeight/3);
	gl.uniform1i(shader.SamplerCor, 1);
	gl.drawArrays(gl.TRIANGLES, 0, triangleVertexPositionBuffer.numItems);

	gl.viewport(gl.viewportWidth/1.5,  0, gl.viewportWidth/3, gl.viewportHeight/3);
	gl.uniform1i(shader.SamplerCor, 2);
	gl.drawArrays(gl.TRIANGLES, 0, triangleVertexPositionBuffer.numItems);

	gl.viewport(gl.viewportWidth/3.01,  0, gl.viewportWidth/3, gl.viewportHeight/3);
...
function initTexture(gl, shader) {
	...
	image.onload = function(){
		...
		canvas.height 		= image.height*(4/3);
		gl.viewportWidth 	= canvas.width;
		gl.viewportHeight 	= canvas.height*(3/4) ;
